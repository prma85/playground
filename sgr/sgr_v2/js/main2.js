...
function abrirIndicador(ind, ano) {
    ...
    var content = geraPaginaDados2(id);
    /*if (content) {
        $('#indicador').addClass("active").effect("slide", "slow");
        gauge.updateConfig({
...
function geraPaginaDados2(idIndicador) {
    id = idIndicador;
    //console.log(id);
    var grafico = {}, gPrevisto = [], gRealizado = [];
    //console.log(atual[id]);
    if (atual[id]) {
        ...
        //console.log(atual);
        anomes = indicador.ANOMES;
        var selectAno = geraSelectAno(indicador.ANO);
        var selectMes = geraSelectMes(indicador.ANOMES.substr(-2,6));
        var html = '';
        html += '<div class="container-fluid" id="indicador-details">'
            +'<div class="row margin0">'
            +'      <div class="col-xs-12 col-sm-8 col-lg-8 col-md-8 titulo-indicador">KPI: <span>'+indicador.NOME_INDICADOR+'</span></div>'
            +'      <div class="col-xs-12 col-sm-2 col-lg-1 col-md-1 mes-indicador">Month:</div>'
            +'      <div class="col-xs-12 col-sm-2 col-lg-3 col-md-3 mes-indicador">'+selectMes+'</div>'
            +'</div><br />'
            +'<div class="row margin0">'
            +'      <div class="col-xs-12 col-sm-8 col-lg-8 col-md-8 titulo-indicador">Strategic Objective: <span>'+indicador.OBJETIVO+'</span></div>'
            +'      <div class="col-xs-12 col-sm-2 col-lg-1 col-md-1 mes-indicador">Year:</div>'
            +'      <div class="col-xs-12 col-sm-2 col-lg-3 col-md-3 mes-indicador">'+selectAno+'</div>'
            +'</div>'
            +'<div class="row kpi-results">'
            +'      <div class="col-xs-12 col-sm-6 col-lg-4 col-md-4 meta">KPI Goal: <span>'+indicador.PREVISTO+'</span></div>'
            +'      <div class="col-xs-12 col-sm-6 col-lg-4 col-md-4 realizado">KPI Realized: <span>'+indicador.REALIZADO+'</span></div>'
            +'      <div class="col-xs-12 col-sm-6 col-lg-4 col-md-4 variacao">KPI Variation: <span>'+indicador.VARIACAO+'</span></div>'
            +'</div>'
            +'<div class="row">'
            +'  <div class="box-branco-container-fluid col-xs-12 col-sm-12 col-lg-12">'
            +'      <div id="estado-grafico" class="box-branco demo-container-fluid">'
        ...
        var title = 'KPI details '
        +'<a href="javascript:void(0);" onclick="openHistoric();" class="btn btn-success">Historic</a>'
        +'      <a href="javascript:void(0);" onclick="openFCA();" class="btn btn-success">Results analysis</a>';
...
function geraSelectAno(ano){
  var html = '';
  ano = parseInt(ano);
  html += '<select class="form-control" name="ano" id="indicador-ano">';
  for (i=2012; i<=2017; i++){
    var selected = '';
    if (i==ano){
      selected = 'selected ';
...
function geraSelectMes(mes){
  var html = '';
  html += '<select class="form-control" name="ano" id="indicador-ano">';
  for (i=1; i<=12; i++){
    mesTemp = parseInt(mes);
    var selected = '';
    if (i==mesTemp){
      selected = 'selected ';
    }
    html += '<option '+selected+'value="'+i+'">'+getMes(i)+'</option>';
  }
  html += '</select>';
  return html;
}
    ...
    if (mes != 0 && mes != 13) {
        ...
        if (banco[id].DETALHES[atual[id].ANO].DADOS[anomes].ATINGIDO != ' ') {
            ...
            $.ajax({
                ...
                error: function (err) {
                    console.log(err);
                    alert("The database conection is not active. Try again later!");
                },
...
var tempTitle, tempBody;

function openHistoric(){
    tempBody = $('#myModal .modal-body').html();
    tempTitle = $('#myModal .modal-title').html();

    var div = '<div style="width: 100%;text-align:center;max-width: 700px; margin:0 auto;"><img style="width: 100%;" src="historic.png" /></div>';
    $('#myModal .modal-body').html(div);
    $('#myModal .modal-title').html('12 last months KPI details <a href="javascript:void(0);" onclick="abrirIndicador('+id+');" class="btn btn-success">Back to the KPI</a>');
    $('#myModal').modal('show');
}
...
function openFCA(){
    ...
    var div = '<div class="info">';
        ...
        div = div.concat('</div>');
    $('#myModal .modal-body').html(div);
    $('#myModal .modal-title').html('Results Analysis - Facts, Causes, and Actions  <a href="javascript:void(0);" onclick="abrirIndicador('+id+');" class="btn btn-success">Back to the KPI</a>');
    $('#myModal').modal('show');
}

function goBack(){
    $('#myModal .modal-body').html(tempBody);
    $('#myModal .modal-title').html(tempTitle);
    $('#myModal').modal('show');
}
