...
$.getJSON("http://" + window.location.host + "/apps/webservice/sgr/todos_indicadores.json", function (data) {  
  banco = data;
});
    ...
    if (mes != 0 && mes != 13) {
        ...
        if (banco[id].DETALHES[atual[id].ANO].DADOS[anomes].ATINGIDO != ' ') {
            ...
            $.ajax({
                ...
                url: "http://" + window.location.host + "/apps/webservice/sgr/mapa/" + id + "/" + anomes,
                type: 'GET',
                async: false,
                cache: false,
                timeout: 30000,
...
function geraPaginaDadosUnidade(unidade, idUn, anomesUn) {
    ...
    var grafico = {}, gPrevisto = [], gRealizado = [];
    $.ajax({
        url: "http://" + window.location.host + "/apps/webservice/sgr/unidade/" + idUn + "/" + unidade + "/" + anomesUn,
        type: 'GET',
        async: false,
        cache: false,
        timeout: 30000,
...
function abreDetalhes() {
    ...
    $.ajax({
        ...
        url: "http://" + window.location.host + "/apps/webservice/sgr/indicador/" + id,
        type: 'GET',
        async: false,
        cache: false,
        timeout: 30000,
