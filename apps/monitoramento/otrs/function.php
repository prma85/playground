...
FROM
  ...
  WHERE t.type_id        =ts.id(+)
  AND t.service_id       =s.id(+)
  AND t.ticket_state_id NOT IN (2,3,21)
  and t.title not like 'Host DOWN alert for%'
  order by 1
  ) t
";
if ($matricula) {
    $sql .= " where t.matricula = '" . $matricula . "'";
}
...
/* Declara��o dos arrays de objetos */
...
GLOBAL $fechadosMes;
GLOBAL $pendentes;
getData($sql, $matricula);

//echo $sql;
//debug para corre��o de erros
...
function getData($sql, $matricula = null) {
    GLOBAL $abertos;
    GLOBAL $fechadosHoje;
    GLOBAL $fechadosMes;
    GLOBAL $pendentes;
    ...
    if ($conn) {
        ...
        $sql_fechados_hoje = "select count(t.id) as total from ticket t, users u, COL_COLABORADOR col where ticket_state_id in (2,3,21) and TO_CHAR(t.change_time,'DD/MM/YYYY') = TO_CHAR(sysdate,'DD/MM/YYYY') and t.RESPONSIBLE_USER_ID = u.ID(+) and u.LOGIN = col.COL_DSC_LOGIN (+)";
        if ($matricula) {
            $sql_fechados_hoje .= " and col.COL_COD_MATRICULA = '" . $matricula . "'";
        }
        $fechadosHoje = retornaOracle($conn, $sql_fechados_hoje);

        $sql_fechados_mes = "select count(t.id) as total from ticket t, users u, COL_COLABORADOR col where ticket_state_id in (2,3,21) and TO_CHAR(t.change_time,'MM/YYYY') = TO_CHAR(sysdate,'MM/YYYY') AND t.change_time<=LAST_DAY(SYSDATE) and t.RESPONSIBLE_USER_ID = u.ID(+) and u.LOGIN = col.COL_DSC_LOGIN (+)";
        if ($matricula) {
            $sql_fechados_mes .= " and col.COL_COD_MATRICULA = '" . $matricula . "'";
        }
        $fechadosMes = retornaOracle($conn, $sql_fechados_mes);
