...
class sgrModel {
    ...
    public function getIndicadoresRecentes($mode = 'object') {
        $sql = "SELECT DISTINCT obj.obj_dsc_objetivo objetivo,
            ...
            FC_STATUS_INDICADOR_MENSAL2(NVL(IDD.IDD_VLR_META, 0), NVL(CIN.CIN_VLR_VALOR, 0), NVL(
          ...
          FROM cin_consulta_indicador cin,
            ...
            WHERE IND_COD_INDICADOR IN (15666,5902,5903,14666,25169,2315,15386,14908,17906,14667,19047,19046,1480,6685,12842,15647,6962,1482,19626,19666,11423,17866,19566)
    ...
    public function getUnidadesMapa($idCapital, $idInterior, $anomes) {
        $sql = "select IDD.IND_COD_INDICADOR INDICADOR,
                ...
                IND.IND_SENTIDO_INDICADOR SENTIDO,
                FC_STATUS_INDICADOR_MENSAL2(NVL(IDD.IDD_VLR_META, 0), NVL(CIN.CIN_VLR_VALOR, 0), 
    ...
    public function getIndicadoresHtml($indicadores) {
        $objetivos = array('414' => 'verde', '415' => 'verde', '416' => 'amarelo', '417' => 'amarelo', '418' => 'vermelho', '419' => 'vermelho', '420' => 'azul', '421' => 'azul', '423' => 'roxo', '422' => 'roxo');
        $html = array();
        $page = 1;
        $html[] = '<div class="container page_indicador show" data-id="'.$page.'" id="page_'.$page.'"><div class="row">';
        $ind_js = array();
        foreach ($indicadores as $k => $i) {
            ...
            //$html[] = '<div class="titulo"><a title="link para o indicador '.$i->INDICADOR.'" href="javascript:abrirIndicador('.$i->INDICADOR.', '.$ano.');" >'.$i->NOME_INDICADOR.'</a></div>';
            $html[] = '<div class="titulo"><a title="link para o indicador ' . $i->INDICADOR . '" href="javascript:abrirIndicador(' . $i->INDICADOR . ');" >' . $i->NOME_INDICADOR . '</a></div>';
            ...
            $html[] = '<a title="link para o indicador ' . $i->INDICADOR . '" href="javascript:abrirIndicador(' . $i->INDICADOR . ');" ><img alt="' . $i->STATUS . '" src="img/' . $i->STATUS . '.png"></a>';
            ...
            if ($j % 12 == 0 && array_key_exists($j+1, $indicadores)) {
                $html[] = '</div></div>';
                $page++;
                $html[] = '<div class="container page_indicador" data-id="'.$page.'" id="page_'.$page.'"><div class="row">';
            }
