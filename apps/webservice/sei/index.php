...
/* Estado */
$app->get('/estado', function () {
    GLOBAL $app;
    $app->response()->header('Content-Type', 'application/json;charset=utf-8');

    GLOBAL $i;
    $ano = $_SERVER['QUERY_STRING'];
    echo json_encode($i->estado($ano));
});

/* Negocio */
$app->get('/negocio', function () {
    GLOBAL $app;
    $app->response()->header('Content-Type', 'application/json;charset=utf-8');

    GLOBAL $i;
    $ano = $_SERVER['QUERY_STRING'];
    echo json_encode($i->negocio($ano));
});

/* Localidade */
$app->get('/localidade', function () {
    GLOBAL $app;
    $app->response()->header('Content-Type', 'application/json;charset=utf-8');

    GLOBAL $i;
    $ano = $_SERVER['QUERY_STRING'];
    echo json_encode($i->localidade($ano));
});
...
$app->get('/cidades', function () {
    ...
    GLOBAL $i;
    echo json_encode($i->cidades());
});

/* Cidades */
$app->get('/localidadesuns', function () {
    GLOBAL $app;
    $app->response()->header('Content-Type', 'application/json;charset=utf-8');
       

    GLOBAL $i;
    $result = $i->localidadesUNs();
    $fp = fopen('localidadesuns.json', 'w');
...
/* TESTE LISTA ANOS */
$app->get('/anos', function () {
    GLOBAL $app;
    $app->response()->header('Content-Type', 'application/json;charset=utf-8');

    GLOBAL $i;
    echo json_encode($i->listaMesAno());
});
...
$app->get('/json/files', function () {
    ...
    //grava arquivo de dados do estado
    $fp = fopen('estado.json', 'w');
    fwrite($fp, json_encode($result['ESTADO']));
    fclose($fp);

    //grava arquivo de unidades de negocio
    $fp = fopen('negocio.json', 'w');
    fwrite($fp, json_encode($result['NEGOCIO']));
    fclose($fp);

    //grava arquivo de localidades
    $fp = fopen('localidade.json', 'w');
    fwrite($fp, json_encode($result['LOCALIDADE']));
    fclose($fp);

    //grava arquivo de cidades
    $fp = fopen('cidades.json', 'w');
    fwrite($fp, json_encode($result['CIDADES']));
    fclose($fp);

    //grava arquivo de localidades das UNS
    $fp = fopen('localidadesuns.json', 'w');
    fwrite($fp, json_encode($result['LOCALIDADESUNS']));
    fclose($fp);
    ...
    $texto .= '<a href="../estado.json">Baixar banco JSON de dados do estado</a><br>';
    $texto .= '<a href="../negocio.json">Baixar banco JSON de unidades de negócio</a><br>';
    $texto .= '<a href="../localidade.json">Baixar banco JSON de localidades</a><br>';
    $texto .= '<a href="../cidades.json">Baixar banco JSON de cidades</a><br>';
    $texto .= '<a href="../localidadesuns.json">Baixar banco JSON de localidades das UNs</a><br>';
