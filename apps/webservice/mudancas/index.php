...
<!DOCTYPE html>
    <html>
        ...
        <body>
            ...
            <h1>Bem vindo a API dos dados do OTRS da CAGECE!</h1>
            <section>
                <h2>Começando</h2>
                <p><b>URI: /json</b><br/>
...
/* result */
$app->get('/result/:query', function ($query) {
	GLOBAL $app;
	$app->response()->header('Content-Type', 'application/json;charset=utf-8');
	
	GLOBAL $i;
	echo json_encode($i->result($query));
});

/* backup */
$app->get('/backup/:query', function ($query) {
	GLOBAL $app;
	$app->response()->header('Content-Type', 'application/json;charset=utf-8');
	
	GLOBAL $i;
	echo json_encode($i->backup($query));
});

/* ativoafetado */
$app->get('/afetado/:id', function ($id) {
	GLOBAL $app;
	$app->response()->header('Content-Type', 'application/json;charset=utf-8');
	
	GLOBAL $i;
	echo json_encode($i->afetado($id));
});

/* ativoafetado */
$app->get('/faq/:id', function ($id) {
	GLOBAL $app;
	$app->response()->header('Content-Type', 'application/json;charset=utf-8');
	
	GLOBAL $i;
	echo json_encode($i->faq($id));
});

/* ativoafetado */
$app->get('/chamados/:id', function ($id) {
	GLOBAL $app;
	$app->response()->header('Content-Type', 'application/json;charset=utf-8');
	
	GLOBAL $i;
	echo json_encode($i->chamados($id));
});
